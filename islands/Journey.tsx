...
export default function Journey(props: { onNext(): void, onBack(): void }) {
    ...
    return (
        <div class={tw`bg-smoke text-black`}>
            ...
            <main>
                ...
                <section class={tw`my-12`}>
                    <p class={tw`font-mono mb-1 text-${_color}-600`}>Hi! let ya'll kno me</p>
                    <h1 class={tw`font-bold tracking-wider text-dark`}>Darwin Prayoga</h1>
                    <h2 class={tw`font-bold mb-3 tracking-wider text-sub`}>Build up growth things</h2>
                ...
                <section class={tw`grid grid-cols-3 mb-6`}>
                    ...
                    <div class={tw`col-span-1 ml-2`}>
                        <img class={tw`w-full h-full bg-${_color}-600 border(white 2) shadow-lg rounded-2xl`} src='/profile.png' />
                    </div>
                </section>
                ...
                <section class={tw`mb-24`}>
                    ...
                    <div class={tw`flex mb-6 justify-between items-center bg-white p-4 shadow-lg rounded-2xl w-full`}>
                        ...
                        <button onClick={() => open('https://biofip.com')} class={tw`focus:outline-none flex gap-2 bg-${_color}-600 rounded-xl py-1 px-2 text-white`}>OPEN<Right class={tw`fill-current w-5`} /></button>
                    </div>
                    <div class={tw`flex mb-6 justify-between items-center bg-white p-4 shadow-lg rounded-2xl w-full`}>
                        <menu class={tw`justify-start items-center`}>
                        ...
                        <button onClick={props.onNext} class={tw`focus:outline-none flex gap-2 bg-${_color}-600 rounded-xl py-1 px-2 text-white`}>OPEN<Right class={tw`fill-current w-5`} /></button>
                    </div>

                    <div class={tw`flex justify-end`}>
                        <h2 class={tw`font-bold mb-6 tracking-wider text-default`}>Work Experience</h2>
                    </div>
                    <div class={tw`flex mb-6 justify-between items-center bg-white p-4 shadow-lg rounded-2xl w-full`}>
                        <menu class={tw`justify-start items-center`}>
                            <img src='/victory.png' class={tw`w-12 mr-4`} />
                            <menuitem class={tw`gap-0`}>
                                <h3>Victory International</h3>
                                <p>Global financial investment and trading facilities, <sub>as Digital Marketing</sub></p>
                            </menuitem>
                        </menu>
                        <button onClick={() => open('https://vifx.co.id')} class={tw`focus:outline-none flex gap-2 bg-${_color}-600 rounded-xl py-1 px-2 text-white`}>OPEN<Right class={tw`fill-current w-5`} /></button>
                    </div>
                    <div class={tw`flex mb-6 justify-between items-center bg-white p-4 shadow-lg rounded-2xl w-full`}>
                        <menu class={tw`justify-start items-center`}>
                        ...
                        <button onClick={() => open('https://ibftrader.com')} class={tw`focus:outline-none flex gap-2 bg-${_color}-600 rounded-xl py-1 px-2 text-white`}>OPEN<Right class={tw`fill-current w-5`} /></button>
                    </div>
